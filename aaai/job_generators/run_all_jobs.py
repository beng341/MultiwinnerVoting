...
#SBATCH --output=slurm_out/n_voters=%{n_voters}_m=%{m}_k=%{num_winners}_pref_dist=%{pref_model}.out
...
echo "Starting network evaluation..."
...
def generate_jobs():
    folder_path = "/home/jcaiata/projects/def-klarson/jcaiata/MultiwinnerVoting/aaai/cc_jobs/combined_jobs"
    ...
    for m, pref_model in itertools.product(M_VALUES, PREF_MODELS):
        ...
        for num_winners in range(1, m):
            ...
            filename = f"cc_job_combined_n_profiles={N_PROFILES}_n_voters={N_VOTERS}_m={m}_k={num_winners}_pref_dist='{pref_model}'_axioms='custom'.sh"
            filepath = os.path.join(folder_path, filename)
            
            with open(filepath, "w") as f:
                print(f"Writing job to {filepath}")
                f.write(job_content)
