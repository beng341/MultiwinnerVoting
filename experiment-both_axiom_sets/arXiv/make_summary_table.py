...
def make_summary_table(n_profiles=[], num_voters=[], m_set=[], k_set=[], pref_dist=[], axioms=[]):
    ...
    res_count = 0
    result_folder = 'evaluation_results-AAAI_combined'
    ...
    rule_sortorder = []
    if "NN-all" in summ_stats.keys():
        rule_sortorder.append("NN-all")
    if "NN-root" in summ_stats.keys():
        rule_sortorder.append("NN-root")
    else:
        pass
    if ("NN-all" not in summ_stats.keys()) and ("NN-root" not in summ_stats.keys()):
        rule_sortorder.append("NN")
    rule_sortorder += ["Min", "Max",
    ...
    out_path = "experiment-both_axiom_sets/arXiv/summary_tables/"
...
def format_summary_table(n_profiles=[], num_voters=[], m_set=[], k_set=[], pref_dist=[], axioms=[]):
    ...
    root_ax = [evaluation_column_shortnames[f"{rax}-mean"] for rax in root_ax]
    new_col_names = {}
    for col in df.columns:
        if col in root_ax:
            new_col_names[col] = f"\\underline{{{col}}}"
    df = df.rename(columns=new_col_names)

    # Convert to LaTeX
    ncols = len(df.columns)
    # col_alignment = "l" + "c"*(ncols-1)
    col_alignment = "lc|cccccc|ccccccc"
    ...
    midrule_after_values = ["Max", "Bloc", "Greedy M."]
    # add midrule after certain values
    lines = latex_table.split('\n')
    # Find positions of values in the first column
    for value in midrule_after_values:
...
def make_tables_for_all_combinations():
    ...
    for m, pref_dist in itertools.product(m_set, all_pref_dists):
        ...
        make_summary_table(n_profiles, n_voters, [m], k_set, [pref_dist], ["both"])
        format_summary_table(n_profiles, n_voters, [m], k_set, [pref_dist], ["both"])
...
def make_aggregated_table_single_m(m=5):
    ...
    make_summary_table(n_profiles, n_voters, [m], k_set, all_pref_dists, ["both"])
    format_summary_table(n_profiles, n_voters, [m], k_set, all_pref_dists, ["both"])
