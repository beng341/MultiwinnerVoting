...
rule_shortnames = {
    "Neural Network": "NN",
    "Neural Network (all)": "NN-all",
    "Neural Network (root)": "NN-root",
...
rule_markers = {
    "Neural Network": "o",
    "Neural Network (all)": "o",
    "Neural Network (root)": "x",
    "Random Choice": ".",
    "Borda ranking": "d",
...
def generate_plot_data_all_axioms_single_distribution(m=5, dist="IC", metric="std"):
    ...
    :return:
    """
    experiment_folder = "evaluation_results-AAAI_combined"
    ...
    for k in range(1, m):
        ...
        filename = f"axiom_violation_results-n_profiles=25000-num_voters=50-m={m}-k={k}-pref_dist={dist}-axioms=both.csv"
    ...
    for rule, violations in all_rule_violations.items():
        ...
        if "Noise" in rule:
            ...
            rule_label = rule_shortnames[rule[:-6]]     # truncate " Noise" from the rule name
            data_for_plot["series"][rule_label]["noise"] = y_values
            continue
...
def generate_plot_data_each_rule_by_axiom(m=5, rule="Neural Network", dist="IC", metric="std"):
    ...
    for k in range(1, m):
        ...
        for ax in all_axioms.keys():
            ...
            col_name = f"{ax}-mean"
            ax_violation_rate = df.loc[df['Method'] == rule, col_name].values
            if len(ax_violation_rate) == 0:
                print(f"{rule} may not be in loaded data. Skipping.")
                return
            ax_violation_rate = ax_violation_rate[0]
            if ax not in ax_violations:
                ax_violations[ax] = []
            ax_violations[ax].append(ax_violation_rate)

            if "Neural Network" in rule:
                ax_violation_rate = df.loc[df['Method'] == f"{rule} Noise", col_name].values[0]
...
def plot_each_distribution_all_axioms(m, out_folder):
    ...
    fig.supxlabel('Number of Winners', fontsize=12, x=0.5, y=0.13)
    fig.supylabel('Axiom Violation Rate', fontsize=12, x=0.015, y=0.5)

    handles, labels = axs[0, 0].get_legend_handles_labels()
    fig.legend(handles, labels, loc='outside lower center', ncol=7, bbox_to_anchor=(0.5, 0.0))
...
def make_all_plots(m=5):
    out_folder = f"experiment-both_axiom_sets/arXiv/plots/m={m}"
...
if __name__ == "__main__":
    ...
    plot_mixed_distribution_all_axioms_subplots_for_m(out_folder=f"experiment-both_axiom_sets/arXiv/plots")
