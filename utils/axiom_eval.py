...
def eval_majority_axiom(row, rule, tie):
    ...
    """
    rank_choice_normalized = eval(row["rank_matrix-normalized"])
    if tie:
    ...
    top_rated = [rank_choice_normalized[i*num_candidates] for i in range(num_candidates)]
...
def eval_majority_loser_axiom(row, rule, tie):
    ...
    bottom_rated = [rank_choice_normalized[i*num_candidates+num_candidates-1] for i in range(num_candidates)]
...
def eval_condorcet_winner(row, rule, tie):
    ...
    def isCondorcetWinner(winning_committee):
        ...
        for d in not_in_committee:
            ...
            for c in in_committee:
                ...
                if binary_pairs[c * num_candidates + d] == 1:
    ...
    binary_pairs = eval(row["binary_pairs"])
...
def eval_condorcet_loser(rank_counts, winning_committee, num_candidates):
    """
    ...
    # A condorcet losing set is a set that for every member d not in the winning committee,
    # there does not exist any member in the committee that is preferred over d by a majority
    # of agents
